...
public class DummyMesh : TerrainMesh
{
  ...
  //       // initialize terrain
  //       targetTerrain[i] = new Vector3(0, 0, 33);
  //       float zTarget = targetTerrain[i].z;
  //       targetMinZ = Mathf.Min(zTarget, targetMinZ);
  //       targetMaxZ = Mathf.Max(zTarget, targetMaxZ);

  //       vertices[i].z = z;
  //       colors[i] = getVertexColor(z, minZ, maxZ, heightmapColors);
  //       // terrainAssistance(ref colors, ref vertices, targetTerrain, i, Mathf.Min(targetMinZ, minZ), Mathf.Max(targetMaxZ, maxZ));
  //     }
  //   }
  ...
  // void terrainAssistance(ref Color32[] colors, ref Vector3[] vertices, Vector3[] targetTerrain, int flattenedIndex, float minZ, float maxZ)
  // {
    
  //   float relativeHeight = vertices[flattenedIndex].z - targetTerrain[flattenedIndex].z;

  //   if (relativeHeight < terrainAssistanceAccomodation && relativeHeight > -terrainAssistanceAccomodation) {
  //       colors[flattenedIndex] = Color.black;
  //   }
  //   else if (relativeHeight < 0)
  //   {
  //     colors[flattenedIndex] = Color.green * -(relativeHeight / (vertices[flattenedIndex].z - minZ));
  //   }
  //   else if (relativeHeight > 0)
  //   {
  //     colors[flattenedIndex] = Color.red * (relativeHeight / (maxZ - vertices[flattenedIndex].z));
  //   };
  // }
  ...
  // Color32 getVertexColor(float z, float minZ, float maxZ, Color32[] heightmapColors)
  // {
