Shader "Unlit/KinectMeshVisalizer"
{
	...
	#define MIN_Z_VALUE 0
	#define MAX_Z_VALUE 3
	...
	{
		// Define a gradient from red to blue
		half4 blueColor = half4(0.0, 0.0, 1.0, 1.0);
		half4 cyanColor = half4(0.0, 1.0, 1.0, 1.0);
		half4 greenColor = half4(0.0, 1.0, 0.0, 1.0);
		half4 yellowColor = half4(1.0, 1.0, 0.0, 1.0);
		half4 redColor = half4(1.0, 0.0, 0.0, 1.0);

		half p = saturate((i.wPos.z - MIN_Z_VALUE) / (MAX_Z_VALUE - MIN_Z_VALUE));
		half4 gradientColor;
	
		if (p < 0.25)
		{
			gradientColor = lerp(blueColor, cyanColor, p*4);
		}
		else if (p < 0.5)
		{
        gradientColor = lerp(cyanColor, greenColor, (p - 0.25) * 4);
		}
		else if (p < 0.75)
		{
			gradientColor = lerp(greenColor, yellowColor, (p - 0.5) * 4);
		}
		else
		{
			gradientColor = lerp(yellowColor, redColor, (p - 0.75) * 4);
		}
